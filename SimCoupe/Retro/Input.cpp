...
int /*SDL_Joystick*/ *pJoystick1, *pJoystick2;
...
static void ReadJoystick (int nJoystick_, int/*SDL_Joystick*/ *pJoystick_, int nTolerance_)
{
#if 0
...
extern int  mbR,mbL,tomx,tomy;

extern "C" void mbup(int x,int y);
extern "C" void mbdown(int x,int y);
...
void retro_joy0(unsigned char joy0){
	...
	// UP  DWN  LEFT RGT  BTN1 BTN0
	// 0    1     2   3    4    5

        int nPosition = 0;  

	if(joy0&0x01)nPosition |= HJ_UP;
	if(joy0&0x08)nPosition |= HJ_RIGHT;
...
bool Input::FilterEvent (int/*SDL_Event*/* pEvent_)
{
...
void Input::Update ()
{
    ...
    Keyboard::SetKey(0/*SDLK_CAPSLOCK*/, false);
    Keyboard::SetKey(0/*SDLK_NUMLOCK*/, false);
}
